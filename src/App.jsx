...
const LOCAL_BACKUP_KEY = "dallas-2026-trip-backup-v1";
...
const saveLocalBackup = (nextData, clientUpdatedAt = Date.now()) => {
...
function App() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(tripDoc, async (snapshot) => {
      ...
      const newestLocalWrite = Math.max(latestLocalWrite.current, localWriteTime);
      ...
      if (snapshot.exists()) {
        ...
        if (!initialCloudLoad.current && backup?.data && localWriteTime > cloudWriteTime) {
          ...
          await setDoc(
            ...
            { merge: true }
          ).catch((error) => console.warn("Could not sync local Dallas backup", error));
        } else if (!initialCloudLoad.current && cloudData.data) {
          const mergedCloudData = mergeData(cloudData.data);
          setData(mergedCloudData);
          saveLocalBackup(mergedCloudData, cloudWriteTime || Date.now());
        } else if (cloudData.data && cloudWriteTime > newestLocalWrite && !justWroteLocally) {
          const mergedCloudData = mergeData(cloudData.data);
          setData(mergedCloudData);
          saveLocalBackup(mergedCloudData, cloudWriteTime || Date.now());
        }
      } else {
        const localData = backup?.data ? mergeData(backup.data) : starter;
        const writeTime = localWriteTime || Date.now();
        latestLocalWrite.current = writeTime;
        setData(localData);
        saveLocalBackup(localData, writeTime);

        await setDoc(tripDoc, {
          data: localData,
          clientUpdatedAt: writeTime,
          updatedAt: serverTimestamp(),
        }).catch((error) => console.warn("Could not create Dallas trip document", error));
      }
  ...
  const writeData = async (nextData) => {
    ...
    latestLocalWrite.current = writeTime;
    setData(nextData);
    saveLocalBackup(nextData, writeTime);
    ...
    await setDoc(
      ...
      { merge: true }
    ).catch((error) => console.warn("Could not save Dallas trip data", error));
  };
