...
const LOCAL_TRIP_BACKUP_KEY = "dallas2026-trip-data-backup-v2";

const canUseStorage = () => typeof window !== "undefined" && typeof window.localStorage !== "undefined";

const readLocalTripBackup = () => {
  if (!canUseStorage()) return null;
  try {
    const saved = window.localStorage.getItem(LOCAL_TRIP_BACKUP_KEY);
    if (!saved) return null;
    const parsed = JSON.parse(saved);
    return parsed?.data ? parsed : null;
  } catch (error) {
    console.warn("Could not read Dallas local backup", error);
    return null;
  }
};

const writeLocalTripBackup = (data, clientUpdatedAt = Date.now(), pending = false) => {
  if (!canUseStorage()) return;
  try {
    window.localStorage.setItem(
      LOCAL_TRIP_BACKUP_KEY,
      JSON.stringify({ data, clientUpdatedAt, pending, savedAt: Date.now() })
    );
  } catch (error) {
    console.warn("Could not save Dallas local backup", error);
  }
};

const markLocalTripBackupSynced = () => {
  if (!canUseStorage()) return;
  try {
    const current = readLocalTripBackup();
    if (!current) return;
    window.localStorage.setItem(
      LOCAL_TRIP_BACKUP_KEY,
      JSON.stringify({ ...current, pending: false, savedAt: Date.now() })
    );
  } catch (error) {
    console.warn("Could not mark Dallas backup synced", error);
  }
};
...
function App() {
  ...
  const localBackupRef = useRef(readLocalTripBackup());
  ...
  useEffect(() => {
    const backup = localBackupRef.current;
    if (backup?.data) {
      latestLocalWrite.current = Math.max(latestLocalWrite.current, num(backup.clientUpdatedAt));
      setData(mergeData(backup.data));
    }
  }, []);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        const backup = localBackupRef.current || readLocalTripBackup();
        const localWriteTime = num(backup?.clientUpdatedAt);
        ...
        if (snapshot.exists()) {
          ...
          if (backup?.data && localWriteTime > cloudWriteTime) {
            const localData = mergeData(backup.data);
            setData(localData);
            latestLocalWrite.current = localWriteTime;
            ...
            try {
              await setDoc(
                ...
                {
                  data: localData,
                  clientUpdatedAt: localWriteTime,
              ...
              markLocalTripBackupSynced();
              localBackupRef.current = { ...backup, data: localData, pending: false };
            } catch (error) {
              console.warn("Dallas local backup is newer but could not sync yet", error);
            }
          } else if (cloudData.data && (!justWroteLocally || !initialCloudLoad.current)) {
            const mergedCloudData = mergeData(cloudData.data);
            setData(mergedCloudData);
            writeLocalTripBackup(mergedCloudData, cloudWriteTime, false);
            localBackupRef.current = { data: mergedCloudData, clientUpdatedAt: cloudWriteTime, pending: false };
          }
        } else {
          const writeTime = localWriteTime || Date.now();
          const startingData = backup?.data ? mergeData(backup.data) : starter;
          latestLocalWrite.current = writeTime;
          setData(startingData);
          writeLocalTripBackup(startingData, writeTime, true);
          localBackupRef.current = { data: startingData, clientUpdatedAt: writeTime, pending: true };
          ...
          try {
            ...
            console.warn("Could not create Dallas cloud document yet", error);
          }
        }
      ...
      (error) => {
        console.warn("Firebase sync failed, using local backup", error);
        const backup = localBackupRef.current || readLocalTripBackup();
        if (backup?.data) setData(mergeData(backup.data));
        setCloudReady(false);
        initialCloudLoad.current = true;
      }
    );
  ...
  const writeData = async (nextData) => {
    ...
    latestLocalWrite.current = writeTime;
    localBackupRef.current = { data: nextData, clientUpdatedAt: writeTime, pending: true };
    writeLocalTripBackup(nextData, writeTime, true);
    setData(nextData);

    if (!cloudReady || (typeof navigator !== "undefined" && !navigator.onLine)) return;
    ...
    markLocalTripBackupSynced();
    localBackupRef.current = { data: nextData, clientUpdatedAt: writeTime, pending: false };
  };
  ...
  const updateData = (updater) => {
    setData((prev) => {
      ...
      if (cloudReady && !(typeof navigator !== "undefined" && !navigator.onLine)) {
        setDoc(
          tripDoc,
          {
            data: nextData,
          ...
          .catch((error) => {
            console.warn("Dallas cloud save failed; local backup kept", error);
          });
      }
  ...
  useEffect(() => {
    const syncPendingBackup = async () => {
      if (!cloudReady || (typeof navigator !== "undefined" && !navigator.onLine)) return;
      const backup = localBackupRef.current || readLocalTripBackup();
      if (!backup?.data || !backup.pending) return;

      try {
        const writeTime = num(backup.clientUpdatedAt) || Date.now();
        const localData = mergeData(backup.data);
        await setDoc(
          tripDoc,
          {
        ...
        markLocalTripBackupSynced();
        localBackupRef.current = { data: localData, clientUpdatedAt: writeTime, pending: false };
      } catch (error) {
        console.warn("Pending Dallas local changes could not sync yet", error);
      }
    };

    syncPendingBackup();
    window.addEventListener("online", syncPendingBackup);
    return () => window.removeEventListener("online", syncPendingBackup);
  }, [cloudReady, tripDoc, user]);
